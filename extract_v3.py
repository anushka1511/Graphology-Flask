        ...
        for j, sum in enumerate(vp):
            ...
            if (sum > 0):
                ...
                for k in range(y2 - y1):
                    row = thresh[int(y1 + k):int(y1 + k + 1), wordStart:wordEnd]  # y1:y2, x1:x2
                    if np.sum(row):
                        count += 1
...
def extractSlant(img, words):
    ...
    for i, angle in enumerate(theta):
        ...
        for j, word in enumerate(words):
            ...
            height = int(word[1] - word[0])
            width = int(word[3] - word[2])
            ...
            # Ensure that the height of original matches the height of new_image
            if original.shape[0] != new_image.shape[0]:
                new_height = new_image.shape[0]
                original = cv2.resize(original, (original.shape[1], new_height))
...
def start(file_path):
    ...
    if image is None:
        raise ValueError(f"Image file '{file_path}' not found or could not be loaded.")
